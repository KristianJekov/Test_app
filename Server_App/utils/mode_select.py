...
def registerate_device_in_mode(mode, driver):
    ...
    if "Deregistered" not in driver.find_element(By.XPATH, config.STATE).text:
        ...
        deregisterate_confirm = driver.find_element(
            By.XPATH, config.DEREGISTERATE_CONFIRM_BTN
        )
    ...
    if mode == "Private":
        select_fourth_registrator = driver.find_element(
            By.XPATH, config.SELECT_FOURTH_REGISTRATOR
        )
        select_fourth_registrator.click()
    else:
        select_first_registrator = driver.find_element(
            By.XPATH, config.SELECT_FIRST_REGISTRATOR
        )
